...
app.get("/auth/me", verifyToken, async (req, res) => {
  try {
    const { data: rows, error } = await supabase
      .from('users')
      .select('id, name, email, gender, bio, location, profile_image, created_at')
      .eq('id', req.userId)
      .limit(1);
...
app.put("/auth/profile", verifyToken, async (req, res) => {
  try {
    ...
    const { error } = await supabase
      .from('users')
      .update({ name, gender, bio, location })
      .eq('id', req.userId);
...
app.post("/auth/login", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Error logging in", message: err.message, stack: err.stack });
  }
});
...
5. Final verdict
    ...
    const { error: histErr } = await supabase.from('history').insert([{ 
      ...
      resume_text_sample: resumeText.substring(0, 500)
    }]);
...
${resumeText}
    ...
    // Save to history
    const { error: histErr2 } = await supabase.from('history').insert([{ 
      user_id: req.userId,
      type: 'tailor',
      job_description: jobDescription,
      result_content: result,
